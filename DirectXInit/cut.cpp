...
void CUT::UpdateSelection(const POINT& clickPosition, const POINT& releasePosition) {
    ...
    int startX = std::min(clickPosition.x, releasePosition.x);
    int startY = std::min(clickPosition.y, releasePosition.y);
    int endX = std::max(clickPosition.x, releasePosition.x);
    int endY = std::max(clickPosition.y, releasePosition.y);
    ...
    // 3x3マス以内の範囲に制限
    endX = std::min(endX, startX + 2);
    endY = std::min(endY, startY + 2);
